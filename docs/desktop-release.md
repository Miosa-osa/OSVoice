...
- During each build the Tauri config is patched so the updater reads the correct channel-specific manifest:
  - Desktop dev channel → `https://github.com/josiahsrc/osvoice/releases/download/desktop-dev/latest.json`
  - Desktop prod channel → `https://github.com/josiahsrc/osvoice/releases/download/desktop-prod/latest.json`
...
- The desktop bundle enables Hardened Runtime with custom entitlements at `apps/desktop/src-tauri/macos/OS Voice.entitlements`. Keep the Developer ID certificate in sync with these capabilities if you adjust microphone or keyboard monitoring features.
...
4. Confirm the dev manifest is accessible at `https://github.com/josiahsrc/osvoice/releases/download/desktop-dev/latest.json`.
...
4. Confirm the production manifest: `https://github.com/josiahsrc/osvoice/releases/download/desktop-prod/latest.json`.
