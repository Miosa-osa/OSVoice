...
This guide explains how to add a new locally-run model to the OS Voice desktop app. It covers the complete flow from UI settings to Rust inference, and explains how to adapt the patterns for different model types.
...
- `OSVOICE_WHISPER_MODEL_URL_{SIZE}` - Override specific model URL
- `OSVOICE_WHISPER_MODEL_URL` - Override base model URL (fallback)
...
- **macOS**: `~/Library/Application Support/OS Voice/models/`
- **Windows**: `C:\Users\{user}\AppData\Roaming\OS Voice\models\`
- **Linux**: `~/.config/OS Voice/models/`
...
fn main() {
    ...
    if std::env::var("OSVOICE_GPU_ENUMERATOR").as_deref() == Ok("1") {
        // Run GPU enumeration and exit
        desktop_lib::system::gpu::run_gpu_enumerator_process();
        return;
    }

    if std::env::var("OSVOICE_KEYBOARD_LISTENER").as_deref() == Ok("1") {
...
fn enumerate_gpus_in_child_process() -> Vec<GpuAdapterInfo> {
    ...
    command
        .env("OSVOICE_GPU_ENUMERATOR", "1")  // Trigger child mode
        .stdin(Stdio::null())
...
```rust
if std::env::var("OSVOICE_MODEL_INFERENCE").as_deref() == Ok("1") {
...
pub fn run_inference_process() -> Result<(), String> {
    ...
    let input: InferenceInput = serde_json::from_str(&std::env::var("OSVOICE_INFERENCE_INPUT")?)?;
...
fn transcribe_in_child(input: &InferenceInput) -> Result<String, String> {
    ...
    let output = Command::new(exe)
        .env("OSVOICE_MODEL_INFERENCE", "1")
        .env("OSVOICE_INFERENCE_INPUT", input_json)
        .output()?;
...
Command::new(exe)
    .env("OSVOICE_KEYBOARD_LISTENER", "1")
    .env("OSVOICE_KEYBOARD_PORT", port.to_string())
    .spawn()?;
...
6. **Environment variables**:
   - `OSVOICE_WHISPER_DISABLE_GPU=1` - Force CPU-only mode
   - `OSVOICE_WHISPER_MODEL_URL_{SIZE}` - Override model download URL
