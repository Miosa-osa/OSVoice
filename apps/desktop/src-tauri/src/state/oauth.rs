...
impl GoogleOAuthConfig {
    pub fn from_env() -> Option<Self> {
        ...
        let client_id = option_env!("OSVOICE_GOOGLE_CLIENT_ID")
            .filter(|s| !s.trim().is_empty())
            .map(|s| s.trim().to_owned())
            .or_else(|| {
                env::var("OSVOICE_GOOGLE_CLIENT_ID")
                    .ok()
                    .filter(|s| !s.trim().is_empty())
                    .map(|s| s.trim().to_owned())
            })?;

        let client_secret = option_env!("OSVOICE_GOOGLE_CLIENT_SECRET")
            .filter(|s| !s.trim().is_empty())
            .map(|s| s.trim().to_owned())
            .or_else(|| {
                env::var("OSVOICE_GOOGLE_CLIENT_SECRET")
                    .ok()
                    .filter(|s| !s.trim().is_empty())
                    .map(|s| s.trim().to_owned())
            })?;
