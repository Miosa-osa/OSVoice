...
pub fn write_startup_diagnostics(app: &tauri::AppHandle) {
    ...
    log_content.push_str(&format!("=== OS Voice Startup Diagnostics ===\n"));
    ...
    if let Ok(val) = std::env::var("OSVOICE_WHISPER_DISABLE_GPU") {
        log_content.push_str(&format!("OSVOICE_WHISPER_DISABLE_GPU: {}\n", val));
    } else {
        log_content.push_str("OSVOICE_WHISPER_DISABLE_GPU: <not set>\n");
    }
    log_content.push_str("\n");
