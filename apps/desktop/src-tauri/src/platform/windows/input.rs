...
pub(crate) fn paste_text_into_focused_field(
    ...
    let override_text = env::var("OSVOICE_DEBUG_PASTE_TEXT").ok();
    let target = override_text.as_deref().unwrap_or(text);
    eprintln!(
        "[osvoice] attempting to inject text ({} chars)",
        target.chars().count()
    );
...
fn is_console_window() -> bool {
    unsafe {
        ...
        eprintln!("[osvoice] foreground window class: {}", class_str);
...
fn send_paste_keys(keybind: Option<&str>) {
    ...
    if is_console && !use_shift {
        eprintln!("[osvoice] detected console window, using right-click to paste");
        send_right_click();
    } else {
