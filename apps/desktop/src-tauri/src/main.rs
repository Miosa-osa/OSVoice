...
fn main() {
    ...
    match app_result {
        Ok(result) => {
            if let Err(err) = result {
                    ...
                    || err_str.contains("GPU")
                {
                    ...
                    eprintln!("[startup] Try setting OSVOICE_WHISPER_DISABLE_GPU=1 to disable GPU acceleration.");
                }
                std::process::exit(1);
            }
        }
        Err(panic_info) => {
            ...
            eprintln!("[startup] If this is a GPU-related crash, try setting OSVOICE_WHISPER_DISABLE_GPU=1");
            std::process::exit(1);
        }
    }
}
