...
const RELEASES_API_URL =
  "https://api.github.com/repos/josiahsrc/osvoice/releases";
...
const ASSET_KEY_MAPPINGS: Array<{
  ...
  {
    match: (name) => /^OSVoice[._](?!GPU).*\.AppImage$/i.test(name),
    keys: ["linux-x86_64", "linux-x86_64-appimage"],
  },
  {
    match: (name) => /^OSVoice[._](?!GPU).*\.deb$/i.test(name),
    keys: ["linux-x86_64-deb"],
  },
  {
    match: (name) => /^OSVoice[._](?!GPU).*\.rpm$/i.test(name),
  ...
  {
    match: (name) => /^OSVoice[._](?!GPU).*\.msi$/i.test(name),
    keys: ["windows-x86_64", "windows-x86_64-msi"],
  },
  {
    match: (name) => /^OSVoice[._](?!GPU).*setup.*\.exe$/i.test(name),
    keys: ["windows-x86_64-nsis"],
  },
  {
    match: (name) => /OSVoice\.GPU.*\.AppImage$/i.test(name),
    keys: ["linux-x86_64-gpu"],
  },
  {
    match: (name) => /OSVoice\.GPU.*\.deb$/i.test(name),
    keys: ["linux-x86_64-gpu-deb"],
  },
  {
    match: (name) => /OSVoice\.GPU.*\.rpm$/i.test(name),
    keys: ["linux-x86_64-gpu-rpm"],
  },
  {
    match: (name) => /OSVoice\.GPU.*\.msi$/i.test(name),
    keys: ["windows-x86_64-gpu"],
  },
  {
    match: (name) => /OSVoice\.GPU.*setup.*\.exe$/i.test(name),
